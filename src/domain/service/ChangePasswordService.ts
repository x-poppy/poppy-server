...
export class ChangePasswordService {
  ...
  async auth(ctx: KoaContext, changePasswordDto: ChangePasswordDto): Promise<StepData> {
    ...
    const user = null; // await this.userRepository.find(userNo);
    if (!user) {
    ...
    const verifyOldPwdResult = await this.passwordService.verify(user.userNo, user.nonce, changePasswordDto.oldPassword, "asdasd");
